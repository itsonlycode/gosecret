...
// It will update the gosecretpw.github.io website and create a new GitHub milestone.
...
func main() {
	...
	if err := updateGopasspw(htmlDir, curVer); err != nil {
		fmt.Printf("Failed to update gosecretpw.github.io: %s\n", err)
	}
	...
	// send PRs to update gosecret ports
...
func newRepoUpdater(client *github.Client, v semver.Version, user, fork string) (*repoUpdater, error) {
	relURL := fmt.Sprintf("https://github.com/gosecretpw/gosecret/releases/download/v%s/gosecret-%s.tar.gz", v.String(), v.String())
	// fetch https://github.com/gosecretpw/gosecret/archive/vVER.tar.gz
	// compute sha256, sha512
	relSHA256, relSHA512, err := checksum(relURL)
	if err != nil {
		return nil, err
	}
	arcURL := fmt.Sprintf("https://github.com/gosecretpw/gosecret/archive/v%s.tar.gz", v.String())
	// fetch https://github.com/gosecretpw/gosecret/archive/vVER.tar.gz
	// compute sha256, sha512
...
func (u *repoUpdater) updateAlpine(ctx context.Context) error {
	...
	r := &repo{
		...
		url: u.arcURL,
		dir: dir,
		msg: "community/gosecret: upgrade to " + u.v.String(),
		rem: u.ghFork,
	}
	...
	// update community/gosecret/APKBUILD
	buildFn := "community/gosecret/APKBUILD"
	buildPath := filepath.Join(dir, buildFn)

	repl := map[string]*string{
		"pkgver=":     strp("pkgver=" + u.v.String()),
		"sha512sums=": strp("sha512sums=\"" + u.arcSHA512 + "  gosecret-" + u.v.String() + ".tar.gz\""),
		"source=":     strp(`source="$pkgname-$pkgver.tar.gz::https://github.com/gosecretpw/gosecret/archive/v$pkgver.tar.gz"`),
	}
...
func (u *repoUpdater) updateHomebrew(ctx context.Context) error {
	...
	// update Formula/gosecret.rb
	buildFn := "Formula/gosecret.rb"
	buildPath := filepath.Join(dir, buildFn)
...
func (u *repoUpdater) updateTermux(ctx context.Context) error {
	...
	// update packages/gosecret/build.sh
	buildFn := "packages/gosecret/build.sh"
	buildPath := filepath.Join(dir, buildFn)
	...
	repl := map[string]*string{
		...
		"TERMUX_PKG_SRCURL":  strp(`TERMUX_PKG_SRCURL=https://github.com/gosecretpw/gosecret/archive/v$TERMUX_PKG_VERSION.tar.gz`),
	}
	if err := updateBuild(
		buildPath,
		repl,
...
func (u *repoUpdater) updateVoid(ctx context.Context) error {
	...
	// update srcpkgs/gosecret/template
	buildFn := "srcpkgs/gosecret/template"
	buildPath := filepath.Join(dir, buildFn)
	...
	repl := map[string]*string{
		...
		"revision=":  nil,
		"distfiles=": strp(`distfiles="https://github.com/gosecretpw/gosecret/archive/v${version}.tar.gz"`),
	}
	if err := updateBuild(
		buildPath,
		repl,
...
type repo struct {
	ver semver.Version // gosecret version
	url string         // gosecret download url
	dir string         // repo dir
	msg string
	rem string // remote
}
...
func (r *repo) branch() string {
	return fmt.Sprintf("gosecret-%s", r.ver.String())
}
...
func (r *repo) commitMsg() string {
	...
	return "gosecret: update to " + r.ver.String()
}
...
func (r *repo) updatePrepare() error {
	...
	// git co -b gosecret-VER
	return r.gitBranch()
}

func (r *repo) updateFinalize(path string) error {
	// git commit -m 'gosecret: update to VER'
	if err := r.gitCommit(path); err != nil {
		return err
	}
	// git push myfork gosecret-VER
